...
void ARAstar::ClearCurrentContainers() {
  //*****清空当前openlist*****//
  std::priority_queue<CellInfo> empty;
  std::swap(empty, current_open_list_);
...
bool ARAstar::AstarAlgorithm(CellInfo& start, CellInfo& goal) {
  ...
  //*****搜索循环*****//
  while (goal.g_value_ > current_open_list_.top().f_value_) {
    //搜索失败
    ...
    //*****弹出当前节点*****//
    CellInfo current_cell_pos = current_open_list_.top();
    current_open_list_.pop();
    ...
    for (int8_t i = 0; i < neighbors.size(); ++i) {
      if (neighbors[i].g_value_ > (current_cell_pos.g_value_ + 1.0f)) {
        ...
        if (!IsInList(neighbors[i].xoy_, close_list)) {
          ...
          current_open_list_.push(neighbors[i]);
        } else {
...
void ARAstar::InconsPushOpenlist(const std::vector<CellInfo>& incons_list_arg) {
  for (int16_t i = 0; i < incons_list_arg.size(); ++i) {
    current_open_list_.push(incons_list_arg[i]);
  }
}
...
void ARAstar::UpdateOpenlisByNewFactor() {
  std::priority_queue<CellInfo> temp_queue;
  std::swap(temp_queue, current_open_list_);

  while (!temp_queue.empty()) {
    current_open_list_.push({temp_queue.top().xoy_,
                             temp_queue.top().g_value_ +
                                 heuristic_factor_ * temp_queue.top().h_value_,
                             temp_queue.top().h_value_,
                             temp_queue.top().g_value_,
                             temp_queue.top().v_value_});
